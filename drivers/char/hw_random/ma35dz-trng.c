...
/*-------------------------------------------------------------------*/
/*  hwrng driver for MA35DZ TRNG                                    */
/*-------------------------------------------------------------------*/

static inline void nu_write_reg(struct ma35dz_trng *tdev, u32 val, u32 reg)
{
	writel_relaxed(val, tdev->reg_base + reg);
}

static inline u32 nu_read_reg(struct ma35dz_trng *tdev, u32 reg)
{
	return readl_relaxed(tdev->reg_base + reg);
}

static inline int ma35dz_trng_wait_busy_clear(struct ma35dz_trng *tdev)
{
...
static int ma35dz_trng_issue_command(struct ma35dz_trng *tdev, int cmd)
{
	unsigned long  timeout;
	int	err;

	err = ma35dz_trng_wait_busy_clear(tdev);
	if (err != 0)
		return err;
...
#ifdef USE_GEN_NONCE
static int ma35dz_trng_gen_nonce(struct ma35dz_trng *tdev, uint32_t *nonce)
{
	int   i, j, loop, err;
	...
	for (i = 0; i < loop; i++) {
		...
		err = ma35dz_trng_issue_command(tdev, TCMD_GEN_NONCE);
		if (err != 0)
			return err;
	}
	return 0;
...
#else
static int ma35dz_trng_gen_noise(struct ma35dz_trng *tdev)
{
	int	err;
	...
	err = ma35dz_trng_issue_command(tdev, TCMD_GEN_NOISE);
	if (err != 0)
		return err;
...
static int ma35dz_trng_create_state(struct ma35dz_trng *tdev)
{
	int	err;
	...
	err = ma35dz_trng_issue_command(tdev, TCMD_CREATE_STATE);
	if (err != 0)
		return err;
...
static int ma35dz_trng_init(struct hwrng *rng)
{
	struct ma35dz_trng *tdev = (struct ma35dz_trng *)rng->priv;
	...
	err = ma35dz_trng_gen_nonce(tdev, nonce);
#else
	pr_debug("TRNG GEN_NOISE...\n");
	err = ma35dz_trng_gen_noise(tdev);
#endif
	if (err != 0)
		return err;

	err = ma35dz_trng_create_state(tdev);
	if (err != 0)
		return err;
...
static int ma35dz_trng_read(struct hwrng *rng, void *buf,
			     size_t max, bool wait)
{
	...
	while (max >= 4) {
		...
		err = ma35dz_trng_issue_command(tdev, TCMD_GEN_RANDOM);
		if (err != 0)
			return err;
...
static int ma35dz_trng_probe(struct platform_device *pdev)
{
	struct device		*dev = &pdev->dev;
	struct ma35dz_trng	*tdev;
	struct resource		*res;
	int			err;
	...
	tdev->rng.init = ma35dz_trng_init;
	tdev->rng.read = ma35dz_trng_read;
...
static int ma35dz_trng_remove(struct platform_device *pdev)
{
...
static const struct of_device_id ma35dz_trng_dt_ids[] = {
	{ .compatible = "nuvoton,ma35dz-trng" },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, ma35dz_trng_dt_ids);

static struct platform_driver ma35dz_trng_driver = {
	.probe		= ma35dz_trng_probe,
	.remove		= ma35dz_trng_remove,
	.driver		= {
		.name		= "nuvoton-trng",
		.of_match_table = ma35dz_trng_dt_ids,
	},
};

module_platform_driver(ma35dz_trng_driver);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("Nuvoton MA35DZ TRNG driver");
