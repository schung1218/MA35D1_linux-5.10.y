...
/*
 * linux/driver/crypto/nuvoton/ma35dz-sha.c
 *
...
static struct nu_sha_dev *ma35dz_sha_find_dev(struct nu_sha_ctx *tctx)
{
...
static int ma35dz_sha_dma_run(struct nu_sha_dev *dd, int is_key_block)
{
	...
	if ((tctx->hash_mode & HMAC_CTL_SHA3EN) && (tctx->bufcnt == 0)) {
		/* workaround for MA35DZ SHA3 in case of DMACNT is 0 */
		ctx->reg_ctl |= HMAC_CTL_DMACSCAD;
	}
 ...
 */
static void  ma35dz_sha_get_result(struct ahash_request *req)
{
 ...
 */
static void ma35dz_sha_finish_req(struct nu_sha_reqctx *ctx, int err)
{
...
static int ma35dz_sha_init(struct ahash_request *req)
{
	struct crypto_ahash *tfm = crypto_ahash_reqtfm(req);
	struct nu_sha_ctx *tctx = crypto_ahash_ctx(tfm);
	struct nu_sha_reqctx *ctx = ahash_request_ctx(req);
	struct nu_sha_dev *dd = ma35dz_sha_find_dev(tctx);
	bool	is_sha3 = false;
	int	klen, plen;
...
static void ma35dz_sha_sg_to_dma_buffer(struct ahash_request *req,
...
static int ma35dz_sha_update_start(struct nu_sha_dev *dd)
{
	struct nu_sha_reqctx *ctx = ahash_request_ctx(dd->req);
	struct nu_sha_ctx *tctx = crypto_tfm_ctx(dd->req->base.tfm);
	int err = 0;
	...
	if ((ctx->req_len > 0) &&  (tctx->bufcnt < ctx->dma_max_size))
		ma35dz_sha_sg_to_dma_buffer(dd->req, tctx, ctx);

	if (ctx->flags & SHA_FLAGS_KEY_BLK) {
		if ((ctx->flags & (SHA_FLAGS_FINUP | SHA_FLAGS_FINAL)) &&
		    (tctx->bufcnt == 0) && (dd->req->nbytes == 0)) {
			pr_err("MA35DZ HMAC does not support 0 data length!\n");
			ma35dz_sha_finish_req(ctx, -EINVAL);
			return -EINVAL;
		}
		err = ma35dz_sha_dma_run(dd, 1);
		if (err != -EINPROGRESS) {
			/* DMA trigger failed, abort! */
			ma35dz_sha_finish_req(ctx, err);
		}
		 ...
		 */
		ma35dz_sha_finish_req(ctx, 0);
		err = 0;
	}
	return err;
}

static int ma35dz_sha_handle_queue(struct nu_sha_dev *dd,
				    struct ahash_request *req)
{
	...
	dd->req = req;
	return ma35dz_sha_update_start(dd);
}

static void ma35dz_sha_dma_complete(struct nu_sha_reqctx *ctx)
{
	struct nu_sha_dev	*dd = ctx->dd;
	struct nu_sha_ctx	*tctx = crypto_tfm_ctx(dd->req->base.tfm);
...
static int ma35dz_sha_update(struct ahash_request *req)
{
	struct nu_sha_reqctx *ctx = ahash_request_ctx(req);
	...
	ma35dz_sha_sg_to_dma_buffer(req, tctx, ctx);
	if (tctx->bufcnt + ctx->req_len <= ctx->dma_max_size)
		return 0;
	return ma35dz_sha_handle_queue(ctx->dd, req);
}

static int ma35dz_sha_final(struct ahash_request *req)
{
	struct nu_sha_reqctx *ctx = ahash_request_ctx(req);
...
static int ma35dz_sha_finup(struct ahash_request *req)
{
	struct nu_sha_reqctx *ctx = ahash_request_ctx(req);
	int err1, err2;
	 ...
	 */
	err2 = ma35dz_sha_final(req);

	return err1 ?: err2;
}

static int ma35dz_sha_digest(struct ahash_request *req)
{
	return ma35dz_sha_init(req) ?: ma35dz_sha_finup(req);
}

static int ma35dz_sha_setkey(struct crypto_ahash *tfm, const u8 *key, u32 keylen)
{
	struct nu_sha_ctx *tctx = crypto_ahash_ctx(tfm);
...
static int ma35dz_sha_export(struct ahash_request *req, void *out)
{
...
static int ma35dz_sha_import(struct ahash_request *req, const void *in)
{
	struct nu_sha_reqctx *ctx = ahash_request_ctx(req);
...
static int ma35dz_sha_cra_init_alg(struct crypto_tfm *tfm, const char *alg_base)
{
	struct nu_sha_ctx *tctx = crypto_tfm_ctx(tfm);
	struct nu_sha_dev *dd = ma35dz_sha_find_dev(tctx);

	dd = ma35dz_sha_find_dev(tctx);
	if (!dd)
		return -ENODEV;
...
static int ma35dz_sha_cra_init(struct crypto_tfm *tfm)
{
	return ma35dz_sha_cra_init_alg(tfm, NULL);
}

static void ma35dz_sha_cra_exit(struct crypto_tfm *tfm)
{
}

static struct ahash_alg  ma35dz_sha_algs[] = {
{
	.init		= ma35dz_sha_init,
	.update		= ma35dz_sha_update,
	.final		= ma35dz_sha_final,
	.finup		= ma35dz_sha_finup,
	.digest		= ma35dz_sha_digest,
	.export		= ma35dz_sha_export,
	.import		= ma35dz_sha_import,
	...
	.halg.base	= {
		...
		.cra_init		= ma35dz_sha_cra_init,
		.cra_exit		= ma35dz_sha_cra_exit,
	}
},
{
	.init		= ma35dz_sha_init,
...
static struct ahash_alg  ma35dz_sha3_algs[] = {
{
	.init		= ma35dz_sha_init,
	.update		= ma35dz_sha_update,
	.final		= ma35dz_sha_final,
...
static void ma35dz_sha_queue_task(unsigned long data)
{
	struct nu_sha_dev *dd = (struct nu_sha_dev *)data;

	ma35dz_sha_handle_queue(dd, NULL);
}

/*
 *  This task is triggerred by Crypto IRQ when a SHA DMA completed.
 */
static void ma35dz_sha_done_task(unsigned long data)
{
	struct nu_sha_dev *dd = (struct nu_sha_dev *)data;
	struct nu_sha_reqctx *ctx = ahash_request_ctx(dd->req);
	struct nu_sha_ctx *tctx = crypto_tfm_ctx(dd->req->base.tfm);
	...
	ma35dz_sha_dma_complete(ctx);
}

int ma35dz_sha_probe(struct device *dev, struct nu_crypto_dev *nu_cryp_dev)
{
	...
	tasklet_init(&sha_dd->done_task, ma35dz_sha_done_task, (unsigned long)sha_dd);
	tasklet_init(&sha_dd->queue_task, ma35dz_sha_queue_task, (unsigned long)sha_dd);
	...
	for (i = 0; i < ARRAY_SIZE(ma35dz_sha_algs); i++) {
		err = crypto_register_ahash(&ma35dz_sha_algs[i]);
		if (err)
			goto err_register;
	}

	for (i = 0; i < ARRAY_SIZE(ma35dz_sha3_algs); i++) {
		err = crypto_register_ahash(&ma35dz_sha3_algs[i]);
		if (err)
			goto err_register;
	}

	pr_info("MA35DZ Crypto SHA engine enabled.\n");
	return 0;
...
err_register:
	...
	for (i = 0; i < ARRAY_SIZE(ma35dz_sha_algs); i++)
		crypto_unregister_ahash(&ma35dz_sha_algs[i]);

	for (i = 0; i < ARRAY_SIZE(ma35dz_sha3_algs); i++)
		crypto_unregister_ahash(&ma35dz_sha3_algs[i]);
...
int ma35dz_sha_remove(struct device *dev,
