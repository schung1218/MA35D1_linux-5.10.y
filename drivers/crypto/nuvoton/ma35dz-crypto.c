...
/*
 * linux/driver/crypto/nuvoton/ma35dz-crypto.c
 *
...
static struct nu_crypto_dev *_ma35dz_crypto_device;

static irqreturn_t ma35dz_crypto_irq(int irq, void *data)
{
...
static int ma35dz_crypto_probe(struct platform_device *pdev)
{
	...
	/* MA35DZ crypto engine clock should be enabled by firmware. */
	...
	_ma35dz_crypto_device = nu_cryp_dev;
	...
	err = devm_request_irq(dev, irq, ma35dz_crypto_irq, IRQF_SHARED,
			       "ma35dz-crypto", nu_cryp_dev);
	if (err) {
		dev_err(dev, "Failed to request IRQ%d: err: %d.\n", irq, err);
		return err;
	}

	err = ma35dz_prng_probe(dev, nu_cryp_dev->reg_base, &nu_cryp_dev->prng);
	if (err)
		dev_err(dev, "failed to init PRNG!\n");

	err = ma35dz_aes_probe(dev, nu_cryp_dev);
	if (err)
		dev_err(dev, "failed to init AES!\n");

	err = ma35dz_sha_probe(dev, nu_cryp_dev);
	if (err)
	...
	err = ma35dz_ecc_probe(dev, nu_cryp_dev);
	if (err)
	...
	err = ma35dz_rsa_probe(dev, nu_cryp_dev);
	if (err)
...
static int ma35dz_crypto_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct nu_crypto_dev *nu_cryp_dev;
	...
	ma35dz_prng_remove(&pdev->dev, nu_cryp_dev->prng);
	ma35dz_aes_remove(&pdev->dev, nu_cryp_dev);
	ma35dz_sha_remove(&pdev->dev, nu_cryp_dev);

#ifdef CONFIG_CRYPTO_ECDH
	ma35dz_ecc_remove(&pdev->dev, nu_cryp_dev);
#endif

#ifdef CONFIG_CRYPTO_RSA
	ma35dz_rsa_remove(&pdev->dev, nu_cryp_dev);
#endif
	devm_kfree(dev, nu_cryp_dev);
	return 0;
}

static const struct of_device_id ma35dz_crypto_of_match[] = {
	{ .compatible = "nuvoton,ma35dz-crypto" },
	{},
};
MODULE_DEVICE_TABLE(of, ma35dz_crypto_of_match);


static struct platform_driver ma35dz_crypto_driver = {
	.probe  = ma35dz_crypto_probe,
	.remove = ma35dz_crypto_remove,
	.driver = {
		.name   = "ma35dz-crypto",
		.owner  = THIS_MODULE,
		.of_match_table = of_match_ptr(ma35dz_crypto_of_match),
	},
};

module_platform_driver(ma35dz_crypto_driver);
