...
/*
 * linux/driver/crypto/nuvoton/ma35hz-crypto.c
 *
...
static struct nu_crypto_dev *_ma35hz_crypto_device;

static irqreturn_t ma35hz_crypto_irq(int irq, void *data)
{
...
static int ma35hz_crypto_probe(struct platform_device *pdev)
{
	...
	/* MA35HZ crypto engine clock should be enabled by firmware. */
	...
	_ma35hz_crypto_device = nu_cryp_dev;
	...
	err = devm_request_irq(dev, irq, ma35hz_crypto_irq, IRQF_SHARED,
			       "ma35hz-crypto", nu_cryp_dev);
	if (err) {
		dev_err(dev, "Failed to request IRQ%d: err: %d.\n", irq, err);
		return err;
	}

	err = ma35hz_prng_probe(dev, nu_cryp_dev->reg_base, &nu_cryp_dev->prng);
	if (err)
		dev_err(dev, "failed to init PRNG!\n");

	err = ma35hz_aes_probe(dev, nu_cryp_dev);
	if (err)
		dev_err(dev, "failed to init AES!\n");

	err = ma35hz_sha_probe(dev, nu_cryp_dev);
	if (err)
	...
	err = ma35hz_ecc_probe(dev, nu_cryp_dev);
	if (err)
	...
	err = ma35hz_rsa_probe(dev, nu_cryp_dev);
	if (err)
...
static int ma35hz_crypto_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct nu_crypto_dev *nu_cryp_dev;
	...
	ma35hz_prng_remove(&pdev->dev, nu_cryp_dev->prng);
	ma35hz_aes_remove(&pdev->dev, nu_cryp_dev);
	ma35hz_sha_remove(&pdev->dev, nu_cryp_dev);

#ifdef CONFIG_CRYPTO_ECDH
	ma35hz_ecc_remove(&pdev->dev, nu_cryp_dev);
#endif

#ifdef CONFIG_CRYPTO_RSA
	ma35hz_rsa_remove(&pdev->dev, nu_cryp_dev);
#endif
	devm_kfree(dev, nu_cryp_dev);
	return 0;
}

static const struct of_device_id ma35hz_crypto_of_match[] = {
	{ .compatible = "nuvoton,ma35hz-crypto" },
	{},
};
MODULE_DEVICE_TABLE(of, ma35hz_crypto_of_match);


static struct platform_driver ma35hz_crypto_driver = {
	.probe  = ma35hz_crypto_probe,
	.remove = ma35hz_crypto_remove,
	.driver = {
		.name   = "ma35hz-crypto",
		.owner  = THIS_MODULE,
		.of_match_table = of_match_ptr(ma35hz_crypto_of_match),
	},
};

module_platform_driver(ma35hz_crypto_driver);
