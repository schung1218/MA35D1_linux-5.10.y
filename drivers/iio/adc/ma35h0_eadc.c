...
struct ma35h0_adc_diff_channel {
	u32 vinp;
	u32 vinn;
};

struct ma35h0_adc_device {
	struct clk	*clk;
	struct clk	*eclk;
	unsigned int	irq;
	void __iomem	*regs;
...
static const struct iio_chan_spec ma35h0_adc_iio_channels[] = {
	ADC_CHANNEL(0, "adc0"),
	ADC_CHANNEL(1, "adc1"),
	ADC_CHANNEL(2, "adc2"),
	ADC_CHANNEL(3, "adc3"),
...
static irqreturn_t ma35h0_trigger_handler(int irq, void *p)
{
	struct iio_poll_func *pf = p;
	struct iio_dev *indio_dev = pf->indio_dev;
	struct ma35h0_adc_device *info = iio_priv(indio_dev);
	int channel;
	int ret_push;
...
static irqreturn_t ma35h0_adc_isr(int irq, void *data)
{
...
static void ma35h0_adc_channels_remove(struct iio_dev *indio_dev)
{
	kfree(indio_dev->channels);
}

static void ma35h0_adc_buffer_remove(struct iio_dev *idev)
{
...
static int ma35h0_adc_chan_of_init(struct iio_dev *indio_dev)
{
	struct device_node *node = indio_dev->dev.of_node;
	struct ma35h0_adc_diff_channel diff[EADC_CH_MAX];
...
static int ma35h0_adc_read_raw(struct iio_dev *indio_dev,
...
static int ma35h0_adc_conf_scan_seq(struct iio_dev *indio_dev,
...
static int ma35h0_adc_update_scan_mode(struct iio_dev *indio_dev,
						const unsigned long *scan_mask)
{
	struct ma35h0_adc_device *info = iio_priv(indio_dev);
	int ret;
	...
	ret = ma35h0_adc_conf_scan_seq(indio_dev, scan_mask);

	return ret;
}

static int __ma35h0_adc_buffer_postenable(struct iio_dev *indio_dev)
{
	struct ma35h0_adc_device *info = iio_priv(indio_dev);
...
static int ma35h0_adc_buffer_postenable(struct iio_dev *indio_dev)
{
	int ret;

	ret = __ma35h0_adc_buffer_postenable(indio_dev);

	return ret;
}

static void __ma35h0_adc_buffer_predisable(struct iio_dev *indio_dev)
{
	struct ma35h0_adc_device *info = iio_priv(indio_dev);
...
static int ma35h0_adc_buffer_predisable(struct iio_dev *indio_dev)
{
	__ma35h0_adc_buffer_predisable(indio_dev);

	return 0;
}

static const struct iio_buffer_setup_ops ma35h0_ring_setup_ops = {
	.postenable = &ma35h0_adc_buffer_postenable,
	.predisable = &ma35h0_adc_buffer_predisable,
};

static const struct iio_info ma35h0_adc_info = {
	.read_raw = &ma35h0_adc_read_raw,
	.update_scan_mode = &ma35h0_adc_update_scan_mode,
};

static int ma35h0_adc_probe(struct platform_device *pdev)
{
	struct iio_dev	*indio_dev;
	struct ma35h0_adc_device *info = NULL;
	int ret = -ENODEV;
	struct resource *res;
	int irq;
	int err = 0;
	...
	indio_dev = devm_iio_device_alloc(&pdev->dev, sizeof(struct ma35h0_adc_device));
	if (indio_dev == NULL) {
	...
	indio_dev->channels = ma35h0_adc_iio_channels;
	...
	ret = request_irq(info->irq, ma35h0_adc_isr,
	...
	ret = ma35h0_adc_chan_of_init(indio_dev);
	if (ret < 0)
		return ret;
	...
	ret = iio_triggered_buffer_setup(indio_dev, &iio_pollfunc_store_time,
					&ma35h0_trigger_handler, &ma35h0_ring_setup_ops);
	if (ret)
	...
	dev_dbg(&pdev->dev, "%s: ma35h0 EADC\n",
		indio_dev->name);
...
err_free_channels:
	/* disable ADCEN */
	writel(readl(info->regs + CTL) & ~ADCEN, info->regs + CTL);
	ma35h0_adc_channels_remove(indio_dev);
err_ret:
	return ret;
}
...
static int ma35h0_adc_remove(struct platform_device *pdev)
{
	...
	ma35h0_adc_buffer_remove(indio_dev);
...
static const struct dev_pm_ops ma35h0_adc_pm_ops = {
	.suspend = ma35h0_adc_suspend,
	.resume = ma35h0_adc_resume,
};
#define MA35D0_ADC_PM_OPS (&ma35h0_adc_pm_ops)
#else
#define MA35D0_ADC_PM_OPS NULL
#endif

#if defined(CONFIG_OF)
static const struct of_device_id ma35h0_eadc_of_match[] = {
	{	.compatible = "nuvoton,ma35h0-eadc" },
	{	},
};
MODULE_DEVICE_TABLE(of, ma35h0_eadc_of_match);
#endif
...
#if defined(CONFIG_OF)
		.of_match_table = of_match_ptr(ma35h0_eadc_of_match),
#endif
	},
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:ma35h0-eadc");
