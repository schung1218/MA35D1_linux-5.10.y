...
/*
 * Nuvoton MA35HZ Clock PLL driver
 *
...
#define to_ma35hz_clk_pll(clk) \
	(container_of(clk, struct ma35hz_clk_pll, clk))
...
static const struct vsipll_freq_conf_reg_tbl ma35hzpll_freq[] = {
...
static void CLK_UnLockReg(struct ma35hz_clk_pll *pll)
{
	int ret;
...
static void CLK_LockReg(struct ma35hz_clk_pll *pll)
{
	/* Lock PLL registers */
...
/* SMIC PLL for CAPLL */
unsigned long CLK_GetPLLFreq_SMICPLL(struct ma35hz_clk_pll *pll,
		...
		unsigned long u64PllFreq, u32 *u32Reg)
{
	...
	if (!((u64PllFreq >= VSIPLL_FCLKO_MIN_FREQ)
		...
		u32Reg[0] = ma35hzpll_freq[0].ctl0_reg;
		u32Reg[1] = ma35hzpll_freq[0].ctl1_reg;
		u64PllClk = ma35hzpll_freq[0].freq;
				...
				unsigned long u64PllFreq, u32 *u32Reg)
{
	...
	if (u64PllFreq > VSIPLL_FCLKO_MAX_FREQ) {
		u32Reg[0] = ma35hzpll_freq[1].ctl0_reg;
		u32Reg[1] = ma35hzpll_freq[1].ctl1_reg;
		u64PllClk = ma35hzpll_freq[1].freq;
		return u64PllClk;
	}
		...
		unsigned long u64PllFreq, u32 u32SR, u32 u32Fmod, u32 *u32Reg)
{
	...
	if (u64PllFreq >= VSIPLL_FCLKO_MAX_FREQ) {
		u32Reg[0] = ma35hzpll_freq[2].ctl0_reg;
		u32Reg[1] = ma35hzpll_freq[2].ctl1_reg;
		u32Reg[2] = ma35hzpll_freq[2].ctl2_reg;
		u64PllClk = ma35hzpll_freq[2].freq;
		return u64PllClk;
	}
...
unsigned long CLK_SetPLLFreq(struct ma35hz_clk_pll *pll,
...
unsigned long CLK_GetPLLFreq_VSIPLL(struct ma35hz_clk_pll *pll,
		unsigned long PllSrcClk)
{
...
static int ma35hz_clk_pll_set_rate(struct clk_hw *hw, unsigned long rate,
		unsigned long parent_rate)
{
	struct ma35hz_clk_pll *pll = to_ma35hz_clk_pll(hw);

	if ((parent_rate < VSIPLL_FREF_MIN_FREQ) || (parent_rate > VSIPLL_FREF_MAX_FREQ))
		return 0;

	if ((pll->type == MA35HZ_CAPLL) || (pll->type == MA35HZ_DDRPLL)) {
		pr_warn("Nuvoton MA35HZ CAPLL/DDRPLL Read Only.\n");
		return 0;
	}
...
static unsigned long ma35hz_clk_pll_recalc_rate(struct clk_hw *hw,
			unsigned long parent_rate)
{
...
static long ma35hz_clk_pll_round_rate(struct clk_hw *hw, unsigned long rate,
					unsigned long *prate)
{
	return rate;
}

static int ma35hz_clk_pll_is_prepared(struct clk_hw *hw)
{
	struct ma35hz_clk_pll *pll = to_ma35hz_clk_pll(hw);
...
static int ma35hz_clk_pll_prepare(struct clk_hw *hw)
{
	struct ma35hz_clk_pll *pll = to_ma35hz_clk_pll(hw);
	u32 val;

	if ((pll->type == MA35HZ_CAPLL) || (pll->type == MA35HZ_DDRPLL)) {
		pr_warn("Nuvoton MA35HZ CAPLL/DDRPLL Enable Only.\n");
		return 0;
	}
...
static void ma35hz_clk_pll_unprepare(struct clk_hw *hw)
{
	struct ma35hz_clk_pll *pll = to_ma35hz_clk_pll(hw);
	u32 val;

	if ((pll->type == MA35HZ_CAPLL) || (pll->type == MA35HZ_DDRPLL))
		pr_warn("Nuvoton MA35HZ CAPLL/DDRPLL Cannot disable.\n");
	else {
		val = __raw_readl(pll->ctl1_base);
		val |= VSIPLLCTL1_PD_MSK;
		__raw_writel(val, pll->ctl1_base);
...
static const struct clk_ops ma35hz_clk_pll_ops = {
	.is_prepared = ma35hz_clk_pll_is_prepared,
	.prepare = ma35hz_clk_pll_prepare,
	.unprepare = ma35hz_clk_pll_unprepare,
	.set_rate = ma35hz_clk_pll_set_rate,
	.recalc_rate = ma35hz_clk_pll_recalc_rate,
	.round_rate = ma35hz_clk_pll_round_rate,
};

struct clk_hw *ma35hz_reg_clk_pll(enum ma35hz_pll_type type,
				u8 u8mode, const char *name,
				const char *parent, unsigned long targetFreq,
				void __iomem *base, struct regmap *regmap)
{
	struct ma35hz_clk_pll *pll;
	struct clk_hw *hw;
